...
import TeacherSupportModal from '~/page-components/support/TeacherSupportModal';
import { ToastContainer } from 'react-toastify';
import SupportDetail from '~/page-components/support/SupportDetail';
import './index.module.scss';
import { getListSupport, getOverviewSupport } from '~/api/teacherAPI';
		...
		try {
			const res = await getListSupport({
				...
				ToDate:
					toDate.length === 0
						? ''
						: dayjs(new Date(toDate)).format('DD/MM/YYYY'), // string dd/mm/yyyy
			});
			if (res.Code === 1) {
				setState(res.Data);
